    ...
    ProcessCharCreation[langcode] = function(chat, user_id, text)
        ...
        for i, var in ipairs(order) do
            if not client.CharCreation[user_id][var[1]] then
                ...
                if order[i + 1] then
                    chat:SendMessage(LANG[langcode][order[i + 1][2]], {reply_markup = {inline_keyboard = {{back}}}})
                    return
                else
                    chat:SendMessage(LANG[langcode]["CHAR_CREATION_FINISH"]:format(client.CharCreation[user_id].id), {reply_markup = {inline_keyboard = {{Private, Public}}}})
                    return
                end
            end
        end
